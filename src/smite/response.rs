...
use crate::{serde_response::*, Response};
...
impl ResponseDatabase {
        ...
        self.heroes.insert(
            *hero_id,
            Hero {
                ...
                img_path: format!("/media/smite/images/{}.png", hero_name),
            },
        );
    ...
    pub async fn populate_responses(&mut self) {
        ...
        tracing::info!("Populating god responses");
        let _ = self.populate_hero_responses().await;
    }

    async fn get_pages_for(&mut self) -> Result<()> {

    }

    async fn smite_populate_hero_responses(&mut self) -> Result<()> {
        let pages = {
            let params = {
